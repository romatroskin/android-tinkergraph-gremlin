        ...
        for (int i = 1; i < steps.size() - 1; i++) {
            ...
            if (!(current instanceof MapStep ||
                    ...
                    (current instanceof TraversalParent &&
                            TraversalHelper.anyStepRecursively(new Predicate<Step>() {
                                @Override
                                public boolean test(Step s) {
                                    return (s instanceof SideEffectStep || s instanceof AggregateStep);
                                }
                            }, (TraversalParent) current)))
                return;
        }

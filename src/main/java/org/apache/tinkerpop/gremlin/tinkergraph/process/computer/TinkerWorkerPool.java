...
public final class TinkerWorkerPool implements AutoCloseable {
    ...
    public void executeVertexProgram(final TriConsumer<Iterator<Vertex>, VertexProgram, TinkerWorkerMemory> worker) throws InterruptedException {
        for (int i = 0; i < this.numberOfWorkers; i++) {
            final int index = i;
            this.completionService.submit(new Callable<Object>() {
                @Override
                public Object call() throws Exception {
                    final VertexProgram vp = TinkerWorkerPool.this.vertexProgramPool.take();
                    final TinkerWorkerMemory workerMemory = TinkerWorkerPool.this.workerMemoryPool.poll();
                    final List<Vertex> vertices = TinkerWorkerPool.this.workerVertices.get(index);
                    worker.accept(vertices.iterator(), vp, workerMemory);
                    TinkerWorkerPool.this.vertexProgramPool.offer(vp);
                    TinkerWorkerPool.this.workerMemoryPool.offer(workerMemory);
                    return null;
                }
            });
        }
    ...
    public void executeMapReduce(final Consumer<MapReduce> worker) throws InterruptedException {
        for (int i = 0; i < this.numberOfWorkers; i++) {
            this.completionService.submit(new Callable<Object>() {
                @Override
                public Object call() throws Exception {
                    final MapReduce mr = TinkerWorkerPool.this.mapReducePool.take();
                    worker.accept(mr);
                    TinkerWorkerPool.this.mapReducePool.offer(mr);
                    return null;
                }
            });
        }

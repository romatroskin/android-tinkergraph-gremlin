...
public final class TinkerGraphComputerView {
    ...
    public TinkerGraphComputerView(final TinkerGraph graph, final GraphFilter graphFilter, final Set<VertexComputeKey> computeKeys) {
        ...
        computeKeys.forEach(new Consumer<VertexComputeKey>() {
            @Override
            public void accept(VertexComputeKey key) {
                TinkerGraphComputerView.this.computeKeys.put(key.getKey(), key);
            }
        });
        ...
        if (this.graphFilter.hasFilter()) {
            graph.vertices().forEachRemaining(new Consumer<Vertex>() {
                ...
                public void accept(Vertex vertex) {
                    ...
                    if (TinkerGraphComputerView.this.graphFilter.hasVertexFilter() && TinkerGraphComputerView.this.graphFilter.legalVertex(vertex)) {
                        TinkerGraphComputerView.this.legalVertices.add(vertex.id());
                        legalVertex = true;
                    }
                    if ((legalVertex || !TinkerGraphComputerView.this.graphFilter.hasVertexFilter()) && TinkerGraphComputerView.this.graphFilter.hasEdgeFilter()) {
                        final Set<Object> edges = new HashSet<>();
                        TinkerGraphComputerView.this.legalEdges.put(vertex.id(), edges);
                        TinkerGraphComputerView.this.graphFilter.legalEdges(vertex).forEachRemaining(new Consumer<Edge>() {
                            @Override
    ...
    public Graph processResultGraphPersist(final GraphComputer.ResultGraph resultGraph,
        ...
        if (GraphComputer.Persist.NOTHING == persist) {
            ...
            if (GraphComputer.ResultGraph.ORIGINAL == resultGraph) {
                ...
                this.graph.vertices().forEachRemaining(new Consumer<Vertex>() {
                    @Override
                    public void accept(Vertex vertex) {
                        final Vertex newVertex = newGraph.addVertex(T.id, vertex.id(), T.label, vertex.label());
                        vertex.properties().forEachRemaining(new Consumer<VertexProperty<Object>>() {
                            @Override
                            public void accept(VertexProperty<Object> vertexProperty) {
                                ...
                                vertexProperty.properties().forEachRemaining(new Consumer<Property<Object>>() {
                                    @Override
            ...
            if (GraphComputer.ResultGraph.ORIGINAL == resultGraph) {
                ...
                this.graph.edges().forEachRemaining(new Consumer<Edge>() {
                    @Override
                    public void accept(Edge edge) {
                        ...
                        edge.properties().forEachRemaining(new Consumer<Property<Object>>() {
                            @Override
                            public void accept(Property<Object> property) {
                                newEdge.property(property.key(), property.value());
                            }
                        });
                    }
                });
    ...
    private void addPropertiesToOriginalGraph() {
        ...
        this.computeProperties.forEach(new BiConsumer<Element, Map<String, List<VertexProperty<?>>>>() {
            @Override
            public void accept(Element element, Map<String, List<VertexProperty<?>>> properties) {
                properties.forEach(new BiConsumer<String, List<VertexProperty<?>>>() {
                    @Override
                    public void accept(String key, List<VertexProperty<?>> vertexProperties) {
                        vertexProperties.forEach(new Consumer<VertexProperty<?>>() {
                            @Override
                            public void accept(VertexProperty<?> vertexProperty) {
    ...
    private void addValue(final Vertex vertex, final String key, final VertexProperty property) {
        final Map<String, List<VertexProperty<?>>> elementProperties = this.computeProperties.computeIfAbsent(vertex, new Function<Element, Map<String, List<VertexProperty<?>>>>() {
            @Override
            public Map<String, List<VertexProperty<?>>> apply(Element k) {
                return new HashMap<>();
            }
        });
        elementProperties.compute(key, new BiFunction<String, List<VertexProperty<?>>, List<VertexProperty<?>>>() {
            @Override
            public List<VertexProperty<?>> apply(String k, List<VertexProperty<?>> v) {

...
public final class TinkerGraphStep<S, E extends Element> extends GraphStep<S, E> implements HasContainerHolder {
    ...
    public TinkerGraphStep(final GraphStep<S, E> originalGraphStep) {
        ...
        originalGraphStep.getLabels().forEach(new Consumer<String>() {
            @Override
            public void accept(String label) {
                TinkerGraphStep.this.addLabel(label);
            }
        });
        ...
        this.setIteratorSupplier(new Supplier<Iterator<E>>() {
            @Override
            public Iterator<E> get() {
                return (Iterator<E>) (Vertex.class.isAssignableFrom(TinkerGraphStep.this.returnClass) ? TinkerGraphStep.this.vertices() : TinkerGraphStep.this.edges());
            }
        });
    }
        ...
        if (this.ids != null && this.ids.length > 0)
            ...
            return null == indexedContainer ?
                    ...
                    TinkerHelper.queryEdgeIndex(graph, indexedContainer.getKey(), indexedContainer.getPredicate().getValue()).stream()
                            .filter(new Predicate<TinkerEdge>() {
                                @Override
                                public boolean test(TinkerEdge edge) {
                                    return HasContainer.testAll(edge, TinkerGraphStep.this.hasContainers);
                                }
                            })
        ...
        if (this.ids != null && this.ids.length > 0)
            ...
            return null == indexedContainer ?
                    ...
                    IteratorUtils.filter(TinkerHelper.queryVertexIndex(graph, indexedContainer.getKey(), indexedContainer.getPredicate().getValue()).iterator(),
                            new Predicate<TinkerVertex>() {
                                @Override
                                public boolean test(TinkerVertex vertex) {
                                    return HasContainer.testAll(vertex, TinkerGraphStep.this.hasContainers);
                                }
                            });
    }
    ...
    private HasContainer getIndexKey(final Class<? extends Element> indexedClass) {
        ...
        final Iterator<HasContainer> itty = IteratorUtils.filter(hasContainers.iterator(),
                new Predicate<HasContainer>() {
                    @Override
                    public boolean test(HasContainer c) {
                        return c.getPredicate().getBiPredicate() == Compare.eq && indexedKeys.contains(c.getKey());
                    }
                });

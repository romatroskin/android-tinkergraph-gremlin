...
public final class TinkerGraph implements Graph {
    ...
    public <I extends Io> I io(final Io.Builder<I> builder) {
        return (I) builder.graph(this).onMapper(new Consumer<Mapper.Builder>() {
            @Override
            public void accept(Mapper.Builder mapper) {
                mapper.addRegistry(TinkerIoRegistryV1d0.instance());
            }
        }).create();
    }
    ...
    private <T extends Element> Iterator<T> createElementIterator(final Class<T> clazz, final Map<Object, T> elements,
        ...
        if (0 == ids.length) {
            ...
            return clazz.isAssignableFrom(ids[0].getClass()) ?
                    IteratorUtils.filter(IteratorUtils.map(idList, new Function<Object, T>() {
                        @Override
        ...
        return TinkerHelper.inComputerMode(this) ?
                (Iterator<T>) (clazz.equals(Vertex.class) ?
                        IteratorUtils.filter((Iterator<Vertex>) iterator, new Predicate<Vertex>() {
                            @Override
                            public boolean test(Vertex t) {
                                return TinkerGraph.this.graphComputerView.legalVertex(t);
                            }
                        }) :
                        IteratorUtils.filter((Iterator<Edge>) iterator, new Predicate<Edge>() {
                            @Override
                            public boolean test(Edge t) {
                                return TinkerGraph.this.graphComputerView.legalEdge(t.outVertex(), t);
                            }
                        })) :
                iterator;
    }
        ...
        LONG {
            ...
            public Long getNextId(final TinkerGraph graph) {
                return Stream.generate(new Supplier<Long>() {
                    @Override
                    public Long get() {
                        return (graph.currentId.incrementAndGet());
                    }
                }).filter(new Predicate<Long>() {
                    @Override
                    public boolean test(Long id) {
                        return !graph.vertices.containsKey(id) && !graph.edges.containsKey(id);
                    }
                }).findAny().get();
            }
